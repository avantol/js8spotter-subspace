...
There is no installation for JS8Spotter, it may be run in-place after downloading and
unpacking. It is recommended that you keep your files in a JS8Spotter folder under your
documents folder, or somewhere else *outside* the Program Files folder(s). You may
right-click the executable to create a desktop shortcut if you wish.
...
running the software for the first time. You may leave the other settings at
default unless changes are needed. All other options are set via the
menus.
...
before JS8Call may result in undesired behavior.
...
search terms.
   ...
   Example 4: You're an OSINT guy, and want to gather data related to specific
              events, people, or groups. You create a container called "Conspiracy"
              and populate it with search terms related to your intelligence efforts.
...
JS8Spotter watches that TCP traffic, and looks for keywords / search terms that you
...
* The "Matched Activity" pane will either show the last 100 entries, or a time range
...
* The "Filter" entry box lets you filter what is shown in the activity pane
...
## Add Search Terms Window

Allows you to add/import new or previously saved search terms, or enter search terms in
bulk, line by line.

## Hide Menu Items

The "Hide" items in the View menu allow you to hide/show items in the
Matched Activity pane. These items are still retained in the database, they are
simply hidden on the screen when this option is selected. These options is not
profile specific. These settings are retained when the program is closed.

## Simple Offline Map Window

Provides a simple offline maidenhead grid map of the latest 100 contacts. Maps
are included for certain areas, but new maps may be added or requested.
...
!! It is highly recommended that you read the PDF manual, and examine all files in the forms folder !!!
...
* Use ASCII characters only. A basic text editor (such as notepad++ on Windows)
...
## CommStatOne v1.0.5 statreps

For those who want to participate in CommStat status reports, but don't want to
or can't install CommStatOne, JS8Spotter includes basic statrep functionality.
In order for this to function, you must first set a CommStat group in the
settings. Then you can send, receieve, and forward statreps with other members
of your group.
